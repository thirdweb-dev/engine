...
export class BackendWalletService {
  ...
  public create(requestBody?: {
    label?: string;
    /**
     * Type of new wallet to create. It is recommended to always provide this value. If not provided, the default
     * wallet type will be used.
     */
    type?:
      | "local"
      | "aws-kms"
   ...
   * Remove an existing backend wallet. NOTE: This is an irreversible action for local wallets. Ensure any funds are
   * transferred out before removing a local wallet.
   * @param walletAddress A contract or wallet address
   * @returns any Default Response
   * @throws ApiError
   */
  public removeBackendWallet(walletAddress: string): CancelablePromise<{
    result: {
      status: string;
    };
  }> {
  ...
  public import(
    requestBody?: {
      ...
      | {
          ...
          awsKmsArn: string;
          /**
           * Optional AWS credentials to use for importing the wallet, if not provided, the default AWS credentials will be
           * used (if available).
           */
          credentials?: {
            /**
      ...
      | {
          ...
          /**
           * Optional GCP credentials to use for importing the wallet, if not provided, the default GCP credentials will be
           * used (if available).
           */
          credentials?: {
            /**
  ...
  /**
   * Get nonce
   * Get the last used nonce for this backend wallet. This value managed by Engine may differ from the onchain value.
   * Use `/backend-wallet/reset-nonces` if this value looks incorrect while idle.
   * @param chain A chain ID ("137") or slug ("polygon-amoy-testnet"). Chain ID is preferred.
   * @param walletAddress Backend wallet address
   * @returns any Default Response
   * @throws ApiError
