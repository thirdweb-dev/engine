...
export class Erc721Service {
   ...
   * Check if tokens are currently available for claiming, optionally specifying if a specific wallet address can
   * claim.
   * @param quantity The amount of tokens to claim.
   * @param chain A chain ID ("137") or slug ("polygon-amoy-testnet"). Chain ID is preferred.
   * @param contractAddress Contract address
   * @param addressToCheck The wallet address to check if it can claim tokens. This considers all aspects of the active
   *   claim phase, including allowlists, previous claims, etc.
   * @returns any Default Response
   * @throws ApiError
   */
  ...
  /**
   * Get claimer proofs
   * Returns allowlist information and merkle proofs for a given wallet address. Returns null if no proof is found for
   ...
   * Approve or remove operator as an operator for the caller. Operators can call transferFrom or safeTransferFrom for
  ...
  /**
   * Generate signature
   * Generate a signature granting access for another wallet to mint tokens from this ERC-721 contract. This method is
   * typically called by the token contract owner.
   * @param chain A chain ID ("137") or slug ("polygon-amoy-testnet"). Chain ID is preferred.
   * @param contractAddress ERC721 contract address
   * @param xBackendWalletAddress Backend wallet address
   * @param xIdempotencyKey Transactions submitted with the same idempotency key will be de-duplicated. Only the last
  ...
  public erc721SignatureGenerate(
    ...
    requestBody?:
      | {
          /**
           * The wallet address that can use this signature to mint tokens. This is to prevent another wallet from
           * intercepting the signature and using it to mint tokens for themselves.
           */
          to: string;
          /**
           * The address that will receive the royalty fees from secondary sales. Defaults to the royaltyRecipient address
           * of the contract.
           */
          ...
          royaltyBps?: number;
          /**
           * If a price is specified, the funds will be sent to the primarySaleRecipient address. Defaults to the
           * primarySaleRecipient address of the contract.
           */
          ...
          metadata:
            ...
            | string;
          /**
           * The address of the currency to pay for minting the tokens (use the price field to specify the price). Defaults
      ...
      | {
          ...
          price?: string;
          /**
           * The amount of the "currency" token this token costs in wei. Remember to use the correct decimals amount for
           * the currency. Example: "100000000000000000" = 0.1 ETH (18 decimals)
           */
          priceInWei?: string;
          /**
          ...
          currency?: string;
          /**
           * If a price is specified, funds will be sent to the "primarySaleRecipient" address. Defaults to the
           * "primarySaleRecipient" address of the contract.
           */
          primarySaleRecipient?: string;
          /**
           * The address that will receive the royalty fees from secondary sales. Defaults to the "royaltyRecipient"
          ...
          /**
           * The uid is a unique identifier hashed in the payload to prevent replay attacks, ensuring it's only used once
           * on-chain.
           */
          uid?: string;
        },
   ...
   * Overwrite the claim conditions for the drop. All properties of a phase are optional, with the default being a
   * free, open, unlimited claim, in the native currency, starting immediately.
   * @param chain A chain ID ("137") or slug ("polygon-amoy-testnet"). Chain ID is preferred.
   * @param contractAddress Contract address
   * @param xBackendWalletAddress Backend wallet address
   * @param requestBody
   ...
   * Update a single claim phase, by providing the index of the claim phase and the new phase configuration. The index
   * is the position of the phase in the list of phases you have made, starting from zero. e.g. if you have two phases,
   * the first phase has an index of 0 and the second phase has an index of 1. All properties of a phase are optional,
   * with the default being a free, open, unlimited claim, in the native currency, starting immediately.
   * @param chain A chain ID ("137") or slug ("polygon-amoy-testnet"). Chain ID is preferred.
   * @param contractAddress Contract address
   * @param xBackendWalletAddress Backend wallet address
   * @param requestBody
