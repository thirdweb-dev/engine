...
export const getTransactionsToProcess = async (database: Knex) : Promise<number> => {
...
export const updateTransactionState = async (
  ...
  txResponse?: providers.TransactionResponse | undefined) : Promise<any> => {
    ...
    try {
      ...
      if (state == TransactionState.processed) {
        ...
        const error = createCustomError(`Invalid transaction state: ${state}`, 500, "INVALID_TRANSACTION_STATE");
        throw error;
      }
